#include <stdlib.h>
#include <stdio.h>
#include <unistd.h>
#include <string.h>
#include "lib/parsing.h"
#include "lib/utils.h"
#include "lib/run.h"

char* prompt = "520sh> ";

void one_iter(char* cmd_input_buf, struct many_commands* cmds, int batch_mode,
        FILE* fp) {

    char* newline = "\n";

    if (!batch_mode) {
        if (!write(STDOUT_FILENO, prompt, strlen(prompt))) return;
    }

    cmd_input_buf = read_until_newline(batch_mode, fp);

    if (!cmd_input_buf) return;

    if (batch_mode) {
        if (strlen(cmd_input_buf) > 0) {
            if (!write(STDOUT_FILENO, cmd_input_buf, strlen(cmd_input_buf)))
                return;
        }
        if (!write(STDOUT_FILENO, newline, strlen(newline))) return;
    }

    // go ahead and parse it
    if (!(cmds = parse_input_buffer(cmd_input_buf))) {
        free(cmd_input_buf);
        return;
    }

    // run it
    run_many_commands(cmds);

    // clean up
    free(cmd_input_buf);
    free_many_commands(cmds);
}

int main(int argc, char* argv[]) {

    int argctest = 3
    char* argvtest[4];
    argvtest[0] = "grep";
    argvtest[1] = "'include'";
    argvtest[2] = "para.c";
    argvtest[3] = NULL;
    execvp(argctest, argvtest);

    // handle bad input
    if (argc > 2) {
        do_error();
        exit(1);
    }

    // space to store command, plus null terminator
    char* cmd_input_buf = NULL;
    struct many_commands* cmds;

    if (argc == 2) {
        char* batch_filename = argv[1];
        char mode[2] = "r";
        FILE* fp = fopen(batch_filename, mode);
        if (!fp) {
            do_error();
            exit(1);
        }
        while (!feof(fp)) one_iter(cmd_input_buf, cmds, 1, fp);
        fclose(fp);
    } else {
        while (1) one_iter(cmd_input_buf, cmds, 0, NULL);
    }

    return 0;
}
